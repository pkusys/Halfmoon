...
View::View(const ViewProto& view_proto)
    : id_(gsl::narrow_cast<uint16_t>(view_proto.view_id())),
      ...
      log_space_hash_tokens_(
          static_cast<size_t>(view_proto.log_space_hash_tokens_size()))
{
    for (size_t i = 0; i < engine_node_ids_.size(); i++) {
        engine_node_ids_[i] =
            gsl::narrow_cast<uint16_t>(view_proto.engine_nodes(static_cast<int>(i)));
    }
    ...
    absl::flat_hash_set<uint16_t> engine_node_id_set(engine_node_ids_.begin(),
                                                     engine_node_ids_.end());
    DCHECK_EQ(engine_node_id_set.size(), num_engine_nodes);

    absl::flat_hash_set<uint16_t> sequencer_node_id_set(sequencer_node_ids_.begin(),
                                                        sequencer_node_ids_.end());
    DCHECK_EQ(sequencer_node_id_set.size(), num_sequencer_nodes);

    absl::flat_hash_set<uint16_t> storage_node_id_set(storage_node_ids_.begin(),
                                                      storage_node_ids_.end());
    ...
    for (size_t i = 0; i < num_engine_nodes; i++) {
        ...
        for (size_t j = 0; j < userlog_replicas_; j++) {
            uint16_t storage_node_id =
                gsl::narrow_cast<uint16_t>(view_proto.storage_plan(
                    static_cast<int>(i * userlog_replicas_ + j)));
    ...
    for (size_t i = 0; i < num_engine_nodes; i++) {
        ...
        engines_.push_back(Engine(
            this,
            node_id,
            NodeIdVec(storage_nodes[node_id].begin(), storage_nodes[node_id].end()),
    ...
    for (size_t i = 0; i < num_storage_nodes; i++) {
        ...
        storages_.push_back(Storage(this,
                                    node_id,
...
View::Engine::Engine(const View* view,
                     uint16_t node_id,
...
View::Sequencer::Sequencer(const View* view,
                           uint16_t node_id,
    ...
    : view_(view),
      ...
      replica_sequencer_node_set_(replica_sequencer_nodes.begin(),
                                  replica_sequencer_nodes.end()),
      index_engine_node_set_(index_engine_nodes.begin(), index_engine_nodes.end()),
      next_index_engine_node_(0)
{}

View::Storage::Storage(const View* view,
                       uint16_t node_id,
                       const View::NodeIdVec& source_engine_nodes)
    : view_(view),
      node_id_(node_id),
      source_engine_nodes_(source_engine_nodes),
      source_engine_node_set_(source_engine_nodes.begin(), source_engine_nodes.end())
{}
