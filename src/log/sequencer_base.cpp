...
void
SequencerBase::StopInternal()
{}

void
SequencerBase::SetupZKWatchers()
{
...
void
SequencerBase::MessageHandler(const SharedLogMessage& message,
...
void
SequencerBase::ReplicateMetaLog(const View* view, const MetaLogProto& metalog)
{
    ...
    for (uint16_t sequencer_id: sequencer_node->GetReplicaSequencerNodes()) {
        bool success =
            SendSharedLogMessage(protocol::ConnType::SEQUENCER_TO_SEQUENCER,
                                 sequencer_id,
                                 message,
                                 STRING_AS_SPAN(payload));
        if (!success) {
            HLOG_F(ERROR,
                   "Failed to send metalog message to sequencer {}",
                   sequencer_id);
        }
    }
}
...
void
SequencerBase::PropagateMetaLog(const View* view, const MetaLogProto& metalog)
{
    uint32_t logspace_id = metalog.logspace_id();
    DCHECK_EQ(bits::LowHalf32(logspace_id), my_node_id());
    ...
    case MetaLogProto::NEW_LOGS:
        for (size_t i = 0; i < view->num_engine_nodes(); i++) {
            ...
            uint32_t shard_delta =
                metalog.new_logs_proto().shard_deltas(static_cast<int>(i));
            if (shard_delta > 0) {
    ...
    for (uint16_t storage_id: storage_nodes) {
        bool success = SendSharedLogMessage(protocol::ConnType::SEQUENCER_TO_STORAGE,
                                            storage_id,
                                            message,
                                            STRING_AS_SPAN(payload));
        if (!success) {
            HLOG_F(ERROR,
                   "Failed to send metalog message to storage {}",
                   storage_id);
        }
    }
}
...
bool
SequencerBase::SendSequencerMessage(uint16_t sequencer_id,
                                    SharedLogMessage* message,
                                    std::span<const char> payload)
{
    message->origin_node_id = node_id_;
...
bool
SequencerBase::SendEngineResponse(const SharedLogMessage& request,
...
void
SequencerBase::OnRecvSharedLogMessage(int conn_type,
                                      ...
                                      std::span<const char> payload)
{
    ...
    DCHECK((conn_type == kSequencerIngressTypeId &&
           ...
           (conn_type == kStorageIngressTypeId &&
            op_type == SharedLogOpType::SHARD_PROG))
...
bool
SequencerBase::SendSharedLogMessage(protocol::ConnType conn_type,
                                    ...
                                    std::span<const char> payload)
{
    ...
    EgressHub* hub = CurrentIOWorkerChecked()->PickOrCreateConnection<EgressHub>(
        ...
        absl::bind_front(&SequencerBase::CreateEgressHub,
                         this,
                         conn_type,
                         dst_node_id));
    if (hub == nullptr) {
...
void
SequencerBase::OnRemoteMessageConn(const protocol::HandshakeMessage& handshake,
                                   int sockfd)
{
    ...
    connection->SetNewMessageCallback(
        IngressConnection::BuildNewSharedLogMessageCallback(
            absl::bind_front(&SequencerBase::OnRecvSharedLogMessage,
                             this,
...
void
SequencerBase::OnConnectionClose(ConnectionBase* connection)
{
...
EgressHub*
SequencerBase::CreateEgressHub(protocol::ConnType conn_type,
                               uint16_t dst_node_id,
                               IOWorker* io_worker)
{
