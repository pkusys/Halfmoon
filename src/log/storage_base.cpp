...
void
StorageBase::SetupZKWatchers()
{
    view_watcher_.SetViewCreatedCallback([this](const View* view) {
...
void
StorageBase::MessageHandler(const SharedLogMessage& message,
...
SerializedLogEntry(const LogEntry& log_entry)
{
    ...
    log_entry_proto.mutable_user_tags()->Add(log_entry.user_tags.begin(),
...
std::optional<LogEntryProto>
StorageBase::GetLogEntryFromDB(uint64_t seqnum)
{
...
StorageBase::PutLogEntryToDB(const LogEntry& log_entry)
{
    ...
    db_->Put(bits::HighHalf64(seqnum),
...
void
StorageBase::LogCachePutAuxData(uint64_t seqnum, std::span<const char> data)
{
...
std::optional<std::string>
StorageBase::LogCacheGetAuxData(uint64_t seqnum)
{
...
bool
StorageBase::SendSequencerMessage(uint16_t sequencer_id,
                                  SharedLogMessage* message,
                                  std::span<const char> payload)
{
    message->origin_node_id = node_id_;
...
bool
StorageBase::SendEngineResponse(const SharedLogMessage& request,
...
void
StorageBase::OnRecvSharedLogMessage(int conn_type,
                                    ...
                                    std::span<const char> payload)
{
    ...
    DCHECK(
        ...
        (conn_type == kEngineIngressTypeId &&
         op_type == SharedLogOpType::SET_AUXDATA))
...
bool
StorageBase::SendSharedLogMessage(protocol::ConnType conn_type,
                                  ...
                                  std::span<const char> payload3)
{
    ...
    EgressHub* hub = CurrentIOWorkerChecked()->PickOrCreateConnection<EgressHub>(
        ...
        absl::bind_front(&StorageBase::CreateEgressHub,
                         this,
                         conn_type,
                         dst_node_id));
    if (hub == nullptr) {
...
void
StorageBase::OnRemoteMessageConn(const protocol::HandshakeMessage& handshake,
                                 int sockfd)
{
    ...
    connection->SetNewMessageCallback(
        IngressConnection::BuildNewSharedLogMessageCallback(
            absl::bind_front(&StorageBase::OnRecvSharedLogMessage,
                             this,
...
void
StorageBase::OnConnectionClose(ConnectionBase* connection)
{
...
EgressHub*
StorageBase::CreateEgressHub(protocol::ConnType conn_type,
                             uint16_t dst_node_id,
                             IOWorker* io_worker)
{
