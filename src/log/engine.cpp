...
Engine::Engine(engine::Engine *engine)
    : EngineBase(engine), log_header_(fmt::format("LogEngine[{}-N]: ", my_node_id())),
      current_view_(nullptr), current_view_active_(false) {}
...
void Engine::OnViewFinalized(const FinalizedView *finalized_view) {
    ...
    Index::QueryResultVec query_results;
    {
        ...
        producer_collection_.ForEachActiveLogSpace(
            ...
            [finalized_view, &append_results](uint32_t logspace_id,
                ...
                log_utils::FinalizedLogSpace<LogProducer>(producer_ptr, finalized_view);
        ...
        index_collection_.ForEachActiveLogSpace(
            finalized_view->view(),
            [finalized_view, &query_results](uint32_t logspace_id, LockablePtr<Index> index_ptr) {
    ...
    if (!append_results.empty()) {
        SomeIOWorker()->ScheduleFunction(nullptr, [this, results = std::move(append_results)] {
            ProcessAppendResults(results);
        });
    }
    if (!query_results.empty()) {
        SomeIOWorker()->ScheduleFunction(nullptr, [this, results = std::move(query_results)] {
...
namespace {
static Message BuildLocalReadOKResponse(uint64_t seqnum, std::span<const uint64_t> user_tags,
                                        std::span<const char> log_data) {
    Message response = MessageHelper::NewSharedLogOpSucceeded(SharedLogResultType::READ_OK, seqnum);
    if (user_tags.size() * sizeof(uint64_t) + log_data.size() > MESSAGE_INLINE_DATA_SIZE) {
        LOG_F(FATAL, "Log data too large: num_tags={}, size={}", user_tags.size(), log_data.size());
    }
...
static Message BuildLocalReadOKResponse(const LogEntry &log_entry) {
    return BuildLocalReadOKResponse(log_entry.metadata.seqnum, VECTOR_AS_SPAN(log_entry.user_tags),
...
#define ONHOLD_IF_SEEN_FUTURE_VIEW(LOCAL_OP_VAR)                                                   \
    do {                                                                                           \
        ...
        if (current_view_ == nullptr || view_id > current_view_->id()) {                           \
            future_requests_.OnHoldRequest(view_id, SharedLogRequest(LOCAL_OP_VAR));               \
            return;                                                                                \
        }                                                                                          \
    } while (0)
...
void Engine::HandleLocalAppend(LocalOp *op) {
    ...
    HVLOG_F(1, "Handle local append: op_id={}, logspace={}, num_tags={}, size={}", op->id,
            op->user_logspace, op->user_tags.size(), op->data.length());
...
void Engine::HandleLocalRead(LocalOp *op) {
    DCHECK(op->type == SharedLogOpType::READ_NEXT || op->type == SharedLogOpType::READ_PREV ||
           op->type == SharedLogOpType::READ_NEXT_B);
    HVLOG_F(1, "Handle local read: op_id={}, logspace={}, tag={}, seqnum={}", op->id,
            op->user_logspace, op->query_tag, bits::HexStr0x(op->seqnum));
...
void Engine::HandleLocalSetAuxData(LocalOp *op) {
    ...
    Message response =
        MessageHelper::NewSharedLogOpSucceeded(SharedLogResultType::AUXDATA_OK, seqnum);
...
#define IGNORE_IF_FROM_PAST_VIEW(MESSAGE_VAR)                                                      \
    do {                                                                                           \
        if (current_view_ != nullptr && (MESSAGE_VAR).view_id < current_view_->id()) {             \
            HLOG(WARNING) << "Receive outdate request from view " << (MESSAGE_VAR).view_id;        \
            return;                                                                                \
        }                                                                                          \
    } while (0)
...
void Engine::HandleRemoteRead(const SharedLogMessage &request) {
    ...
    DCHECK(op_type == SharedLogOpType::READ_NEXT || op_type == SharedLogOpType::READ_PREV ||
...
void Engine::OnRecvNewMetaLogs(const SharedLogMessage &message, std::span<const char> payload) {
...
void Engine::OnRecvNewIndexData(const SharedLogMessage &message, std::span<const char> payload) {
    HVLOG(1) << "recv index";
    DCHECK(SharedLogMessageHelper::GetOpType(message) == SharedLogOpType::INDEX_DATA);
    IndexDataProto index_data_proto;
    if (!index_data_proto.ParseFromArray(payload.data(), static_cast<int>(payload.size()))) {
...
void Engine::OnRecvResponse(const SharedLogMessage &message, std::span<const char> payload) {
    ...
    if (result == SharedLogResultType::READ_OK || result == SharedLogResultType::EMPTY ||
        ...
        if (result == SharedLogResultType::READ_OK) {
            ...
            FinishLocalOpWithFailure(op, SharedLogResultType::EMPTY, message.user_metalog_progress);
        ...
        if (cached_aux_data.has_value()) {
            ...
            if (full_size <= MESSAGE_INLINE_DATA_SIZE) {
                ...
                HLOG_F(WARNING,
                       ...
                       bits::HexStr0x(seqnum), log_entry.data.size(), log_entry.user_tags.size(),
                       cached_aux_data->size());
            }
        }
        if (local_request) {
            ...
            SendReadResponse(query, &response, VECTOR_AS_CHAR_SPAN(log_entry.user_tags),
...
void Engine::ProcessIndexContinueResult(const IndexQueryResult &query_result,
    ...
    HVLOG_F(1, "Process IndexContinueResult: next_view_id={}", query_result.next_view_id);
    const IndexQuery &query = query_result.original_query;
    const View::Sequencer *sequencer_node = nullptr;
    LockablePtr<Index> index_ptr;
    {
    ...
    if (index_ptr != nullptr) {
        ...
        if (!send_success) {
            uint32_t logspace_id =
                bits::JoinTwo16(sequencer_node->view()->id(), sequencer_node->node_id());
    ...
    for (const IndexQueryResult &result : results) {
        ...
        case IndexQueryResult::kEmpty:
            if (query.origin_node_id == my_node_id()) {
                ...
                SendReadFailureResponse(query, SharedLogResultType::EMPTY, result.metalog_progress);
            }
            break;
...
SharedLogMessage Engine::BuildReadRequestMessage(const IndexQueryResult &result) {
    ...
    SharedLogMessage request = SharedLogMessageHelper::NewReadMessage(query.DirectionToOpType());
...
IndexQuery Engine::BuildIndexQuery(LocalOp *op) {
    return IndexQuery{
        ...
        .prev_found_result = {.view_id = 0, .engine_id = 0, .seqnum = kInvalidLogSeqNum}};
}
...
IndexQuery Engine::BuildIndexQuery(const SharedLogMessage &message) {
    ...
    return IndexQuery{.direction = IndexQuery::DirectionFromOpType(op_type),
                      ...
                      .prev_found_result = IndexFoundResult{.view_id = message.prev_view_id,
