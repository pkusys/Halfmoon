...
namespace faas { namespace protocol {
...
public:
    ...
    static FuncCall NewWithMethod(uint16_t func_id,
                                  uint16_t method_id,
                                  uint16_t client_id,
                                  uint32_t call_id)
    {
...
struct Message {
    ...
    union {
        uint64_t
            parent_call_id; // [8:16]  Used in INVOKE_FUNC, saved as full_call_id
        struct {
            int32_t dispatch_delay;  // [8:12]  Used in FUNC_CALL_COMPLETE,
                                     // FUNC_CALL_FAILED
            int32_t processing_time; // [12:16] Used in FUNC_CALL_COMPLETE
        } __attribute__((packed));
        uint64_t log_seqnum; // [8:16]  Used in SHARED_LOG_OP
        // uint64_t txn_id;     // [8:16]  Used in SHARED_LOG_OP
    };
...
inline std::string
EncodeHandshakeMessage(ConnType type, uint16_t src_node_id = 0)
{
...
public:
    ...
    {
        return static_cast<MessageType>(message.message_type) ==
               MessageType::LAUNCHER_HANDSHAKE;
    }

    static bool IsFuncWorkerHandshake(const Message& message)
    {
        return static_cast<MessageType>(message.message_type) ==
               MessageType::FUNC_WORKER_HANDSHAKE;
    }
    ...
    static int32_t ComputeMessageDelay(const Message& message)
    {
        if (message.send_timestamp > 0) {
            return gsl::narrow_cast<int32_t>(GetMonotonicMicroTimestamp() -
...
#define NEW_EMPTY_MESSAGE(MSG_VAR) \
    Message MSG_VAR;               \
    memset(&MSG_VAR, 0, sizeof(Message))

    static Message NewLauncherHandshake(uint16_t func_id)
    {
        NEW_EMPTY_MESSAGE(message);
        message.message_type =
            static_cast<uint16_t>(MessageType::LAUNCHER_HANDSHAKE);
    ...
    static Message NewFuncWorkerHandshake(uint16_t func_id, uint16_t client_id)
    {
        NEW_EMPTY_MESSAGE(message);
        message.message_type =
            static_cast<uint16_t>(MessageType::FUNC_WORKER_HANDSHAKE);
        message.func_id = func_id;
    ...
    static Message NewHandshakeResponse(uint32_t payload_size)
    {
        NEW_EMPTY_MESSAGE(message);
        message.message_type =
            static_cast<uint16_t>(MessageType::HANDSHAKE_RESPONSE);
    ...
    static Message NewCreateFuncWorker(uint16_t client_id)
    {
        NEW_EMPTY_MESSAGE(message);
        message.message_type =
            static_cast<uint16_t>(MessageType::CREATE_FUNC_WORKER);
        message.client_id = client_id;
        return message;
    }

    static Message NewInvokeFunc(const FuncCall& func_call,
    ...
    static Message NewDispatchFuncCall(const FuncCall& func_call)
    {
        NEW_EMPTY_MESSAGE(message);
        message.message_type =
            static_cast<uint16_t>(MessageType::DISPATCH_FUNC_CALL);
        SetFuncCall(&message, func_call);
        return message;
    }

    static Message NewFuncCallComplete(const FuncCall& func_call,
                                       int32_t processing_time)
    {
        NEW_EMPTY_MESSAGE(message);
        message.message_type =
            static_cast<uint16_t>(MessageType::FUNC_CALL_COMPLETE);
        SetFuncCall(&message, func_call);
...
#define NEW_EMPTY_GATEWAY_MESSAGE(MSG_VAR) \
    GatewayMessage MSG_VAR;                \
    memset(&MSG_VAR, 0, sizeof(GatewayMessage))
    ...
    static GatewayMessage NewFuncCallComplete(const FuncCall& func_call,
                                              int32_t processing_time)
    {
        NEW_EMPTY_GATEWAY_MESSAGE(message);
        message.message_type =
    ...
    static GatewayMessage NewFuncCallFailed(const FuncCall& func_call,
...
#define NEW_EMPTY_SHAREDLOG_MESSAGE(MSG_VAR) \
    SharedLogMessage MSG_VAR;                \
    memset(&MSG_VAR, 0, sizeof(SharedLogMessage))

    static SharedLogMessage* NewEmptySharedLogMessage()
    {
        auto message = new SharedLogMessage();
        memset(message, 0, sizeof(SharedLogMessage));
        return message;
    }
    ...
    static SharedLogMessage NewMetaLogProgressMessage(uint32_t logspace_id,
                                                      uint32_t progress)
    {
        NEW_EMPTY_SHAREDLOG_MESSAGE(message);
    ...
    static SharedLogMessage NewReadAtMessage(uint32_t logspace_id,
