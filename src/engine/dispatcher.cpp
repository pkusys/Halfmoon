...
Dispatcher::Dispatcher(Engine* engine, uint16_t func_id)
    : engine_(engine),
      func_id_(func_id),
      min_workers_(0),
      max_workers_(std::numeric_limits<size_t>::max()),
      ...
      running_workers_stat_(
          stat::StatisticsCollector<uint16_t>::StandardReportCallback(
              fmt::format("running_workers[{}]", func_id))),
      max_concurrency_stat_(
          stat::StatisticsCollector<uint32_t>::StandardReportCallback(
...
bool
Dispatcher::OnFuncWorkerConnected(std::shared_ptr<FuncWorker> func_worker)
{
    ...
    if (requested_workers_.contains(client_id)) {
        ...
        HLOG_F(INFO,
               "FuncWorker (client_id {}) takes {}ms to launch",
               client_id,
...
void
Dispatcher::OnFuncWorkerDisconnected(FuncWorker* func_worker)
{
    DCHECK_EQ(func_id_, func_worker->func_id());
    uint16_t client_id = func_worker->client_id();
    absl::MutexLock lk(&mu_);
...
bool
Dispatcher::OnNewFuncCall(const FuncCall& func_call,
                          ...
                          bool shm_input)
{
    ...
    *dispatch_func_call_message = MessageHelper::NewDispatchFuncCall(func_call);
    if (shm_input) {
        dispatch_func_call_message->payload_size =
    ...
    Tracer::FuncCallInfo* func_call_info =
        engine_->tracer()->OnNewFuncCall(func_call, parent_func_call, input_size);
...
bool
Dispatcher::OnFuncCallCompleted(const FuncCall& func_call,
                                ...
                                size_t output_size)
{
    ...
    DCHECK_EQ(func_id_, func_call.func_id);
    Tracer::FuncCallInfo* func_call_info =
        engine_->tracer()->OnFuncCallCompleted(func_call,
                                               dispatch_delay,
                                               processing_time,
                                               output_size);
...
bool
Dispatcher::OnFuncCallFailed(const FuncCall& func_call, int32_t dispatch_delay)
{
    VLOG(1) << "OnFuncCallFailed " << FuncCallHelper::DebugString(func_call);
    DCHECK_EQ(func_id_, func_call.func_id);
    Tracer::FuncCallInfo* func_call_info =
        engine_->tracer()->OnFuncCallFailed(func_call, dispatch_delay);
    if (func_call_info == nullptr) {
        return false;
    }
    engine_->tracer()->DiscardFuncCallInfo(func_call);
...
void
Dispatcher::FuncWorkerFinished(FuncWorker* func_worker)
{
    uint16_t client_id = func_worker->client_id();
    DCHECK(workers_.contains(client_id));
...
bool
Dispatcher::DispatchPendingFuncCall(FuncWorker* func_worker)
{
    if (pending_func_calls_.empty()) {
        return false;
    }
    double average_processing_time =
        engine_->tracer()->GetAverageProcessingTime(func_id_);
    double max_relative_queueing_delay =
        absl::GetFlag(FLAGS_max_relative_queueing_delay);
    ...
    while (!pending_func_calls_.empty()) {
        ...
        Message* dispatch_func_call_message =
            pending_func_call.dispatch_func_call_message;
        FuncCall func_call = MessageHelper::GetFuncCall(*dispatch_func_call_message);
        if (func_call.client_id == 0 || max_relative_queueing_delay == 0.0 ||
...
void
Dispatcher::DispatchFuncCall(FuncWorker* func_worker,
                             Message* dispatch_func_call_message)
{
    uint16_t client_id = func_worker->client_id();
    DCHECK(workers_.contains(client_id));
...
void
Dispatcher::UpdateWorkerLoadStat()
{
    ...
    HVLOG_F(1,
            "UpdateWorkerLoadStat: running_workers={}, idle_workers={}",
            running_workers,
            idle_workers);
...
size_t
Dispatcher::DetermineExpectedConcurrency()
{
    double average_processing_time =
        engine_->tracer()->GetAverageProcessingTime2(func_id_);
    ...
    if (average_processing_time > 0 && average_instant_rps > 0) {
        double estimated_concurrency =
            absl::GetFlag(FLAGS_expected_concurrency_coef) *
...
size_t
Dispatcher::DetermineConcurrencyLimit()
{
    if (absl::GetFlag(FLAGS_disable_concurrency_limiter)) {
        return max_workers_;
    }
    size_t result = std::numeric_limits<size_t>::max();
    double average_running_delay =
        engine_->tracer()->GetAverageRunningDelay(func_id_);
    double average_instant_rps = engine_->tracer()->GetAverageInstantRps(func_id_);
    if (average_running_delay > 0 && average_instant_rps > 0) {
        double estimated_concurrency = absl::GetFlag(FLAGS_concurrency_limit_coef) *
                                       average_running_delay * average_instant_rps /
                                       1e6;
        estimated_rps_stat_.AddSample(gsl::narrow_cast<float>(average_instant_rps));
        estimated_concurrency_stat_.AddSample(
            gsl::narrow_cast<float>(estimated_concurrency));
...
void
Dispatcher::MayRequestNewFuncWorker()
{
    if (workers_.size() + requested_workers_.size() >= max_workers_) {
        return;
    }
    int64_t current_timestamp = GetMonotonicMicroTimestamp();
    int min_worker_request_interval_ms =
        absl::GetFlag(FLAGS_min_worker_request_interval_ms);
    if (last_request_worker_timestamp_ != -1 && min_worker_request_interval_ms > 0 &&
        current_timestamp >
            last_request_worker_timestamp_ + min_worker_request_interval_ms * 1000)
    {
        return;
    }
    if (absl::GetFlag(FLAGS_always_request_worker_if_possible)) {
        HLOG(INFO)
            << "Request new FuncWorker under always_request_worker_if_possible flag";
    } else {
        size_t expected_concurrency = DetermineExpectedConcurrency();
        if (workers_.size() + requested_workers_.size() >= expected_concurrency) {
            return;
        }
        HLOG(INFO) << "Request new FuncWorker: expected_concurrency="
