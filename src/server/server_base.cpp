...
bool
ServerBase::WithinMyEventLoopThread() const
{
    return base::Thread::current() == &event_loop_thread_;
}

void
ServerBase::ForEachIOWorker(std::function<void(IOWorker* io_worker)> cb) const
{
...
IOWorker*
ServerBase::PickIOWorkerForConnType(int conn_type)
{
...
void
ServerBase::OnRemoteMessageConn(const protocol::HandshakeMessage& handshake,
                                int sockfd)
{
...
void
ServerBase::EventLoopThreadMain()
{
...
ServerBase::SetupIOWorkers()
{
    ...
    for (int i = 0; i < num_io_workers; i++) {
        auto io_worker = std::make_unique<IOWorker>(i,fmt::format("IO-{}", i),
                                                    kDefaultIOWorkerBufferSize);
        int pipe_fds[2] = {-1, -1};
...
ServerBase::SetupMessageServer()
{
    ...
    CHECK(utils::SocketListen(message_sockfd_,
    ...
    auto status = zk_utils::CreateSync(zk_session(),
                                       /* path= */ znode_path,
                                       /* value= */ STRING_AS_SPAN(my_addr),
                                       zk::ZKCreateMode::kEphemeral,
                                       nullptr);
...
void
ServerBase::OnNewMessageConnection(int sockfd)
{
    DCHECK(WithinMyEventLoopThread());
...
void
ServerBase::RegisterConnection(IOWorker* io_worker, ConnectionBase* connection)
{
    ...
    HVLOG(1) << fmt::format(
        "register connection id={} type={:#x} to worker {}(current {})",
        connection->id(),
        connection->type(),
...
void
ServerBase::ListenForNewConnections(int server_sockfd, ConnectionCallback cb)
{
    DCHECK(state_.load() == kBootstrapping);
...
void
ServerBase::DoReadClosedConnection(int pipefd)
{
    DCHECK(WithinMyEventLoopThread());
    while (true) {
...
void
ServerBase::DoAcceptConnection(int server_sockfd)
{
    DCHECK(WithinMyEventLoopThread());
...
Timer*
ServerBase::CreateTimer(int timer_type, IOWorker* io_worker, Timer::Callback cb)
{
...
void
ServerBase::CreatePeriodicTimer(int timer_type,
...
int
ServerBase::GetIngressConnTypeId(protocol::ConnType conn_type, uint16_t node_id)
{
...
int
ServerBase::GetEgressHubTypeId(protocol::ConnType conn_type, uint16_t node_id)
{
    CHECK(kConnTypeIdTable.contains(conn_type));
