...
IOUring*
ConnectionBase::current_io_uring()
{
...
IOWorker::IOWorker(int worker_id,
                   std::string_view worker_name,
                   size_t write_buffer_size)
    : worker_id_(static_cast<size_t>(worker_id)),
...
void
IOWorker::Start(int pipe_to_server_fd)
{
...
void
IOWorker::ScheduleStop()
{
    ScheduleFunction(nullptr, [this] { StopInternal(); });
}

void
IOWorker::WaitForFinish()
{
    DCHECK(state_.load() != kCreated);
...
bool
IOWorker::WithinMyEventLoopThread()
{
    return base::Thread::current() == &event_loop_thread_;
}

void
IOWorker::RegisterConnection(ConnectionBase* connection)
{
    ...
    if (conn_type >= 0) {
        ...
        HLOG_F(INFO,
               "New connection of type {0:#x}, total of type {0:#x} is {1}",
               conn_type,
               connections_by_type_[conn_type]->size());
    }
    if (state_.load(std::memory_order_acquire) == kStopping) {
        HLOG(WARNING)
            << "Receive new connection in stopping state, will close it directly";
        connection->ScheduleClose();
    }
}

void
IOWorker::OnConnectionClose(ConnectionBase* connection)
{
    DCHECK(WithinMyEventLoopThread());
    DCHECK(connections_.contains(connection->id()));
    ...
    if (conn_type >= 0) {
        ...
        HLOG_F(INFO,
               "One connection of type {0} closed, total of type {0} is {1}",
               conn_type,
               connections_by_type_[conn_type]->size());
    }
    ...
    URING_DCHECK_OK(
        io_uring_.Write(pipe_to_server_fd_, data, [this](int status, size_t nwrite) {
            PCHECK(status == 0);
            CHECK_EQ(nwrite, static_cast<size_t>(__FAAS_PTR_SIZE));
            DCHECK_GT(connections_on_closing_, 0);
            connections_on_closing_--;
            if (state_.load(std::memory_order_acquire) == kStopping &&
                connections_.empty() && connections_on_closing_ == 0)
            {
...
void
IOWorker::NewWriteBuffer(std::span<char>* buf)
{
    DCHECK(WithinMyEventLoopThread());
...
void
IOWorker::ReturnWriteBuffer(std::span<char> buf)
{
    DCHECK(WithinMyEventLoopThread());
...
ConnectionBase*
IOWorker::PickConnection(int type)
{
    DCHECK(WithinMyEventLoopThread());
...
void
IOWorker::ScheduleFunction(ConnectionBase* owner, std::function<void()> fn)
{
    if (state_.load(std::memory_order_acquire) != kRunning) {
        HLOG(WARNING)
            << "Cannot schedule function in non-running state, will ignore it";
        return;
    }
    ScheduledFunction function = {.owner_id = (owner == nullptr) ? -1 : owner->id(),
                                  .fn = fn};
...
void
IOWorker::ScheduleIdleFunction(ConnectionBase* owner, std::function<void()> fn)
{
    DCHECK(WithinMyEventLoopThread());
    if (state_.load(std::memory_order_acquire) != kRunning) {
        HLOG(WARNING)
    ...
    idle_functions_.push_back(
        ScheduledFunction{.owner_id = (owner == nullptr) ? -1 : owner->id(),
                          .fn = fn});
}

void
IOWorker::RunScheduledFunctions()
{
    DCHECK(WithinMyEventLoopThread());
    if (state_.load(std::memory_order_acquire) != kRunning) {
        return;
...
void
IOWorker::InvokeFunction(const ScheduledFunction& function)
{
    DCHECK(WithinMyEventLoopThread());
